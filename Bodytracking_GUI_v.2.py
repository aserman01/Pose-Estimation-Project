...
def capture_webcam(window):
    cap = cv2.VideoCapture(0)  # 0 for the default webcam
    while True:
        event, _ = window.read(timeout=20)  # Check for events every 20 milliseconds
...
def mode_1():
    window = sg.Window('Pose Detection Mode 1', mode_1_layout(), finalize=True, element_justification='c', size=(640, 480))
...
def mode_2():
    window = sg.Window('Pose Detection Mode 2', mode_2_layout(), finalize=True, element_justification='c', size=(640, 480))
